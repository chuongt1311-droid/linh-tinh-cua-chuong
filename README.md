...
A standalone script `validate_presets.py` grades canonical players against each preset and reports where they rank. Each midfielder preset, for example, has 4–6 named exemplars (e.g. Anchor Man: Rodri, Casemiro, de Roon, Kamara, Højbjerg; Regista: Vitinha, Frenkie de Jong). The script confirms they cluster in the top quartile of their role's ranking — if Rodri doesn't grade as a top Anchor Man, the preset weights are wrong.

All shipping presets pass validation with median exemplar percentile ≥ 78th (most ≥ 86th). Box-to-Box jumped from 67th to 86th after tuning during development.
...
The repo ships with `PERFECT_scouting_data_2026.csv` baked in. To re-scrape with current numbers:

```bash
python fbref_scraping.py        # FBref + Understat + WhoScored
python sofascore_scraping.py    # Sofascore via ScraperFC
python scouting_model.py        # merges everything into PERFECT_scouting_data_2026.csv
```

The Streamlit app picks up CSV changes automatically — `load_scouts` is keyed on the file's modification time, so the next page refresh re-reads the new data without a cache clear.
...
├── validate_presets.py          # Sanity-check role weights against canonical players
├── fbref_scraping.py            # FBref + Understat + WhoScored scrape
├── sofascore_scraping.py        # Sofascore scrape via ScraperFC
├── scouting_model.py            # Merges the scraped CSVs into PERFECT_scouting_data_2026.csv
├── PERFECT_scouting_data_2026.csv  # Master dataset (regenerated by the scripts)
├── requirements.txt
└── README.md
```
...
- **FBref's position labels can disagree with the eye test.** Several obvious wingers (Olise, Cherki, Foden, Greenwood) are labelled `MF` by FBref because of how they classify wide-mid-in-4-3-3 minutes. The repo has hooks for a manual `POSITION_OVERRIDES` dict if you want to reclassify specific players.
